...
public class WurstplusBlockUtil {
    ...
    public static boolean canSeeBlock(BlockPos p_Pos) {
        return WurstplusBlockUtil.mc.player != null && WurstplusBlockUtil.mc.world.rayTraceBlocks(new Vec3d(WurstplusBlockUtil.mc.player.posX, WurstplusBlockUtil.mc.player.posY + (double)WurstplusBlockUtil.mc.player.getEyeHeight(), WurstplusBlockUtil.mc.player.posZ), new Vec3d((double)p_Pos.getX(), (double)p_Pos.getY(), (double)p_Pos.getZ()), false, true, false) == null;
    }

    public static void placeCrystalOnBlock(BlockPos pos, EnumHand hand) {
        RayTraceResult result = WurstplusBlockUtil.mc.world.rayTraceBlocks(new Vec3d(WurstplusBlockUtil.mc.player.posX, WurstplusBlockUtil.mc.player.posY + (double)WurstplusBlockUtil.mc.player.getEyeHeight(), WurstplusBlockUtil.mc.player.posZ), new Vec3d((double)pos.getX() + 0.5, (double)pos.getY() - 0.5, (double)pos.getZ() + 0.5));
    ...
    public static boolean rayTracePlaceCheck(BlockPos pos, boolean shouldCheck, float height) {
        return !shouldCheck || WurstplusBlockUtil.mc.world.rayTraceBlocks(new Vec3d(WurstplusBlockUtil.mc.player.posX, WurstplusBlockUtil.mc.player.posY + (double)WurstplusBlockUtil.mc.player.getEyeHeight(), WurstplusBlockUtil.mc.player.posZ), new Vec3d((double)pos.getX(), (double)((float)pos.getY() + height), (double)pos.getZ()), false, true, false) == null;
    }

    public static boolean rayTracePlaceCheck(BlockPos pos, boolean shouldCheck) {
        return WurstplusBlockUtil.rayTracePlaceCheck(pos, shouldCheck, 1.0f);
    }

    public static void openBlock(BlockPos pos) {
        EnumFacing[] facings;
        for (EnumFacing f : facings = EnumFacing.values()) {
            Block neighborBlock = WurstplusBlockUtil.mc.world.getBlockState(pos.offset(f)).getBlock();
    ...
    public static boolean placeBlock(BlockPos pos, int slot, boolean rotate, boolean rotateBack, WurstplusSetting setting) {
        if (WurstplusBlockUtil.isBlockEmpty(pos)) {
            EnumFacing[] facings;
            int old_slot = -1;
            ...
            for (EnumFacing f : facings = EnumFacing.values()) {
                ...
                if (emptyBlocks.contains(neighborBlock) || !(WurstplusBlockUtil.mc.player.getPositionEyes(mc.getRenderPartialTicks()).distanceTo(vec) <= 4.25)) continue;
                float[] rot = new float[]{WurstplusBlockUtil.mc.player.rotationYaw, WurstplusBlockUtil.mc.player.rotationPitch};
                if (rotate) {
                    WurstplusBlockUtil.rotatePacket(vec.x, vec.y, vec.z);
                }
                ...
                WurstplusBlockUtil.swingArm(setting);
                if (old_slot != -1) {
    ...
    public static boolean isBlockEmpty(BlockPos pos) {
        try {
            if (emptyBlocks.contains(WurstplusBlockUtil.mc.world.getBlockState(pos).getBlock())) {
                Entity e;
                AxisAlignedBB box = new AxisAlignedBB(pos);
                Iterator entityIter = WurstplusBlockUtil.mc.world.loadedEntityList.iterator();
                do {
    ...
    public static boolean canPlaceBlock(BlockPos pos) {
        if (WurstplusBlockUtil.isBlockEmpty(pos)) {
            EnumFacing[] facings;
            for (EnumFacing f : facings = EnumFacing.values()) {
                if (emptyBlocks.contains(WurstplusBlockUtil.mc.world.getBlockState(pos.offset(f)).getBlock())) continue;
                ...
                if (!(WurstplusBlockUtil.mc.player.getPositionEyes(mc.getRenderPartialTicks()).distanceTo(vec3d) <= 4.25)) continue;
                return true;
            }
        }
        return false;
    ...
    public static void rotatePacket(double x, double y, double z) {
        double diffX = x - WurstplusBlockUtil.mc.player.posX;
        double diffY = y - (WurstplusBlockUtil.mc.player.posY + (double)WurstplusBlockUtil.mc.player.getEyeHeight());
        double diffZ = z - WurstplusBlockUtil.mc.player.posZ;
