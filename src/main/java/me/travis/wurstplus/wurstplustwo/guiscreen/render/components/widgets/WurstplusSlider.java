...
extends WurstplusAbstractWidget {
    ...
    public void render(int master_y, int separe, int absolute_x, int absolute_y) {
        ...
        double mouse = Math.min(this.width, Math.max(0, absolute_x - this.get_x()));
        if (this.click) {
            if (mouse != 0.0) {
                this.setting.set_value(TurokDouble.round(mouse / (double)this.width * (this.setting.get_max(1.0) - this.setting.get_min(1.0)) + this.setting.get_min(1.0)));
            } else {
