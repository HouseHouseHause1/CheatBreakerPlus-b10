...
extends WurstplusPinnable {
    ...
    public String get_totems() {
        try {
            int totems = this.offhand() + this.mc.player.inventory.mainInventory.stream().filter(itemStack -> itemStack.getItem() == Items.TOTEM_OF_UNDYING).mapToInt(ItemStack::getCount).sum();
            if (totems > 1) {
                return "\u00a7a " + totems;
            }
            return "\u00a74 " + totems;
